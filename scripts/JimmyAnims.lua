...
jumping="跳跃",
jumpup="通过跳跃键跳起时的动画",
jumpdown="通过跳跃键跳起后落下的动画",
fall="下落一段时间后播放的动画（默认下落速度为0.6时触发，可通过查找falling变量名来修改）",

sprint = "冲刺",
sprintjumpup="冲刺时通过跳跃键跳起时的动画",
sprintjumpdown="冲刺时通过跳跃键跳起后落下的动画",

crouch = "下蹲",
crouchwalk = "蹲走",
crouchwalkback = "向后蹲走",
crouchjumpup = "下蹲时通过跳跃键跳起时的动画",
crouchjumpdown = "下蹲时通过跳跃键跳起后落下的动画",

elytra = "鞘翅飞行",
elytradown = "鞘翅飞行时向下/俯冲",

trident = "三叉戟激流冲刺动画",
sleep = "睡眠动画",
vehicle = "骑乘任意载具时播放的动画",
swim = "游泳",
...
fly = "创造飞行",
flywalk = "创造飞行移动",
flywalkback = "创造飞行向后移动",
flysprint  = "创造冲刺飞行",
flyup = "创造向上飞",
flydown = "创造向下飞",

climb = "攀爬梯子",
climbstill = "在梯子上没有移动但不蹲住（通常是爬到顶了）",
climbdown = "从梯子向下爬",
climbcrouch = "在梯子上蹲停住",
climbcrouchwalk = "在梯子上蹲住并移动",

water = "在水中但不游泳时播放",
waterwalk = "在水中移动",
waterwalkback = "在水中向后移动",
waterup = "在水中向上",
waterdown = "在水中向下",
watercrouch = "在水中蹲下",
watercrouchwalk = "在水中蹲走",
watercrouchwalkback = "在水中向后蹲走",
watercrouchdown = "在水中蹲着向下",
watercrouchup = "在水中蹲着向上，一般只有灵魂沙水电梯会出现",

freezing = "当玩家在细雪中停留时间达到 140 ticks时触发",
hurt = "循环模式必须为播放一次！受伤动画",
death = "死亡",

    -- Inclusive Animations:

attackR = "循环模式必须为播放一次！使用主手攻击",
attackL = "循环模式必须为播放一次！使用副手攻击",
mineR = "循环模式必须为播放一次！使用主手挖掘",
mineL = "循环模式必须为播放一次！使用副手挖掘",
useR = "循环模式必须为播放一次！使用主手放置方块/使用物品/与方块或生物交互/其他",
useL = "循环模式必须为播放一次！使用副手放置方块/使用物品/与方块或生物交互/其他",
...
--Figura版本检测
--如果Mod为开发版则有可能因为无法比较而报错
--如果您确信你的Mod版本没有问题，请删除下方绿线中间的部分
----------
...
--钓鱼检测变量
local fishing
local oldFishing = fishing
local fishingHook = false

--失明检测
local blind = false
local oldBlind = blind
local blindCheck = false
...
function pings.JimmyAnims_Fishing(x)
    fishingHook = x
end

function pings.JimmyAnims_Blind(x)
    blindCheck = x
end
...
local function anims()
    ...
    if host:isHost() then
        ...
        if fishing ~= oldFishing then
            pings.JimmyAnims_Fishing(fishing)
        end
        oldFishing = fishing

        --失明buff检测
        local statusEffects = host:getStatusEffects()
        blind = false
        for i=1, (#statusEffects) do
            if statusEffects[i]["name"] == "effect.minecraft.blindness" then
                blind = true
                break
            end
        end
        if blind ~= oldBlind then
            pings.JimmyAnims_Blind(blind)
        end
        oldBlind = blind
    end
    ...
    function vehicleTypeCheck(type) --载具检测
        return player:getVehicle():getType() == type
    end

    local vehicleHorseState = sitting and vehicleTypeCheck("minecraft:horse")
    local vehiclePigState = sitting and vehicleTypeCheck("minecraft:pig")
    local vehicleBoatState = sitting and vehicleTypeCheck("minecraft:boat")
    local vehicleState = sitting and not vehicleHorseState and not vehiclePigState and not vehicleBoatState
    ...
    local jumpingState = jumpupState or jumpdownState or crouchjumpupState or crouchjumpdownState or sprintjumpupState or sprintjumpdownState or falling and not elytraState
    ...
    local freezingState = player:getFrozenTicks() == 140    --被冷冻时为true

    local fishingState = fishingHook

    local blindState = blindCheck
    ...
    -- anim play testing
        ...
        if path.jumping then
            path.jumping:playing(excluState and jumpingState)
        end
        ...
        if path.vehicleHorse then
            path.vehicleHorse:playing(excluState and vehicleHorseState)
        end
        if path.vehiclePig then
            path.vehiclePig:playing(excluState and vehiclePigState)
        end
        if path.vehicleBoat then
            path.vehicleBoat:playing(excluState and vehicleBoatState)
        end
        ...
        if path.freezing then path.freezing:playing(incluState and freezingState) end

        if path.fishing then
            path.fishing:playing(incluState and fishingState)
        end

        if path.blind then
            path.blind:playing(incluState and blindState)
        end
    end
...
local function blend(paths, time, itemTime)
    ...
    for _, path in pairs(paths) do
        ...
        if path.vehicleHorse then
            path.vehicleHorse:blendTime(time)
        end
        if path.vehiclePig then
            path.vehiclePig:blendTime(time)
        end
        if path.vehicleBoat then
            path.vehicleBoat:blendTime(time)
        end
        ...
        if path.freezing then path.freezing:blendTime(time) end
        if path.fishing then
            path.fishing:blendTime(time)
        end
        if path.blind then
            path.blind:blendTime(time)
        end
    end
    for _,value in pairs(holdRanims) do
