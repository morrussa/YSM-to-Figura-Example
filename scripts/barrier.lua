...
Barrier = {
    ...
    enable = function (self)
        models.barrier:setVisible(true)
        ...
        if events.RENDER:getRegisteredCount("barrier_render") == 0 then
            events.RENDER:register(function ()
                if not self.IsRenderProcessed and not client:isPaused() then
                    ...
                    for index, modelPart in ipairs(models.barrier.Barrier:getChildren()) do
    ...
    ---バリア機能を無効化する。
    disable = function ()
        models.barrier:setVisible(false)
